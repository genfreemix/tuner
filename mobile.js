...
function TunerApp() {
  ...
  function tick(analyser, sampleRate) {
    ...
    const pitchLong = detectPitch(prepLong, paramsRef.current.rmsThreshold, {
      preferEarly: true,
      minPeak: 0.48
    });
    ...
    const manualLong = paramsRef.current.mode === 'MANUAL' ? detectPitchForTarget(prepLong, paramsRef.current.rmsThreshold * 0.72, manualTargetFreq, {
      maxHarmonic: manualTargetIdx >= 4 ? 1 : 4,
      centsRange: manualTargetIdx >= 4 ? 110 : 150
    }) : null;
    const manualShort = paramsRef.current.mode === 'MANUAL' ? detectPitchForTarget(prepShort, paramsRef.current.rmsThreshold * (manualTargetIdx >= 4 ? 0.42 : 0.7), manualTargetFreq, {
      maxHarmonic: manualTargetIdx >= 4 ? 1 : 4,
    ...
    if (!pitch) {
      ...
      } : paramsRef.current.mode === 'MANUAL' ? mCorrectedPitchForString(pitch.freq, targetFreq, targetIdx >= 4 ? 1 : 4) : autoMatch;
