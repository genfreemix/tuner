  ...
  return (
    <div className={`m-shell theme-${theme}`}>
      ...
      <MobileVU cents={cents} theme={theme} lampColor={lampColor} lampOn={true}
        ...
        springK={0.02 + needleSpeed * 0.16} springD={0.94 - needleSpeed * 0.12} />
...
function TunerApp() {
  ...
  const envRef    = React.useRef({ noiseFloor: 0 });
  ...
  function tick(analyser, sampleRate) {
    ...
    if (!pitch) {
      ...
      if (paramsRef.current.mode === 'MANUAL') {
        ...
        const refined =
          detectPitchForTarget(prepShort, floorGate * 0.6, targetFreq, { maxHarmonic: maxH, centsRange: range }) ||
          detectPitchForTarget(prepLong, floorGate * 0.8, targetFreq, { maxHarmonic: maxH, centsRange: range });
        targetMatch = refined
          ? { freq: refined.freq, cents: refined.cents, harmonic: refined.harmonic, score: refined.score }
          : autoMatch;
      }
      ...
      if (h.length >= (isThin ? 5 : 3)) {
        ...
        if (span <= spanLimit || pitch.clarity > 0.86) {
          ...
          if (inZone && (lockClarity > 0.58 || steadyEnough)) {
          ...
          } else if (Math.abs(c) > 8) {
            // Струну заметно увели (крутят колок) — отпускаем лок, чтобы стрелка
            // снова следовала за высотой и можно было донастроить. Гистерезис в
            // 3 кадра, чтобы одиночный шумовой выброс не сбивал зелёный.
            ...
            // средняя зона (3–8¢) — счётчик выхода затухает, зелёный не мигает
            lockRef.current.outCount = Math.max(0, lockRef.current.outCount - 1);
          }
          // В локе стрелка/ридаут стоят на 0 (струна в строю). Вне лока —
          // следуют за живым значением. Полный сброс — в ветке тишины выше:
          // громкость упала ниже порога (заглушили) → мгновенно Idle.
